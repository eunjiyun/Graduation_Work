...
void CGameFramework::BuildObjects()
{
	...
	m_pStage = new CStage();
	if (m_pStage) m_pStage->BuildObjects(m_pd3dDevice, m_pd3dCommandList);
	...
	for (int i = 0; i < 3; i++) {
		//CAirplanePlayer* pAirplanePlayer = new CAirplanePlayer(m_pd3dDevice, m_pd3dCommandList, m_pStage->GetGraphicsRootSignature());//0213 assertion failed
...
{    
	...
	m_pd3dCommandList->ClearDepthStencilView(d3dDsvCPUDescriptorHandle, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f, 0, 0, NULL);
#endif
	if (m_pPlayer) m_pPlayer->Render(m_pd3dCommandList, m_pStage->GetGraphicsRootSignature(), NULL, m_pCamera);
